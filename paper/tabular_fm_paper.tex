...
\title{Tabular Foundation Models Exhibit State-of-the-Art Cancer Survival Prediction Through Latent Cohort Signals Across Cancer Multiomics}
...
\noindent\textbf{Methods:} We evaluated five classical learners, AutoGluon, TabFM, and four TabPFN generations on 3,238 patients across five cancer cohorts (breast, esophageal, head and neck, lung squamous, and lung adenocarcinoma) partitioned into 400 frozen test sets. Outcomes were 3-year and 5-year overall-survival event classification and an exploratory extreme-survival contrast. Each task used five repeats of five-fold evaluation, validation-only threshold selection, and molecular feature selection confined to the training partition. We compared within-cancer and randomly pooled evaluation and examined cancer-identity, structural-zero, label-permutation, and cancer-held-out diagnostics.

\noindent\textbf{Results:} Under pan-cancer pooling, tabular foundation models achieved a mean ROC AUC of 0.721 for 3-year survival ($n = 645$ patients) and 0.786 for extreme survival ($n = 937$ patients). Within individual cancer cohorts, discrimination averaged a mean ROC AUC of 0.565. However, diagnostic controls revealed that cancer identity alone achieved a mean ROC AUC of 0.707, while patterns of missing data alone reached 0.745 without using molecular measurements. Furthermore, evaluating models on an unseen, held-out cancer cohort dropped discrimination to chance (mean ROC AUC 0.479).
...
\label{tab:cohorts}
\input{tables/generated/table_01_cohort_characteristics.tex}
\end{table}
...
\label{tab:model_auc}
\resizebox{\linewidth}{!}{\input{tables/generated/table_02_model_auc_summary.tex}}
\end{table}
...
\label{tab:shortcuts}
\input{tables/generated/table_03_shortcut_summary.tex}
\end{table}
